...
You need to open this document in Github using the web browser to see PDF files. The simulated times are divided into two panes based on the observed response type. The =np= model didn't have the issue of simulating the max survival time (=31).

- [[file:chessB_pn/hist_time.pdf][=np= model]]
