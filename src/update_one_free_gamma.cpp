...
// we draw a single free gamme gamma_s
    ...
    // std::cout << "Drawing...\n";
    gamma_s = stan::math::lognormal_rng(stan::math::log(gamma(c)), jump_gamma(c), rng);
            ...
            for (int i = r.rows().begin(); i < r.rows().end(); ++i) {
              for (int k = r.cols().begin(); k < r.cols().end(); ++k) {
                if (NA(i, k) == 1) {
                  ...
                  if (Y(i, k) == c) {
                    running_total += (gamma_s - gamma(c)) *
                                     stan::math::distance(z.row(c * N + k),
                                                          w.row(c * I + i));
                  }
                }
              }
    ...
    // std::cout << "updating gamma...\n" << "logr_gamma: " << logr_gamma << "gamma and c" << gamma(c) << ", " << c << std::endl;
}
